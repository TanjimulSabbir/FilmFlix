...
export default function SimilarDetails({ movie, handleCloseModal }) {
    ...
    return (
        <div className="w-full md:w-2/3 fixed mx-auto inset-0 flex items-center justify-center z-[60]">
            <div className='flex flex-col space-y-10 md:space-y-0 md:flex-row md:space-x-10 justify-center shadow-2xl rounded-2xl bg-[#2e2e2e]'>
                ...
                <div className='relative w-full px-2 py-7'>
                    <p className="text-3xl font-extrabold">{original_title}</p>
                    ...
                    <div className='w-full flex flex-wrap items-center gap-5'>
                        <button onClick={() => setOpenTrailer(!openTrailer)}
                            className='min-w-fit flex items-center space-x-1 py-2 px-3 md:px-6 border border-green-500 rounded-xl bg-green-500 transition-all duration-500 hover:bg-green-600 justify-center'

                        >
                            <PiPlayCircleFill />
                            <span> {openTrailer ? "Quit Trailer" : "Watch Trailer"}</span>
                        </button>
                        <Link to={`/movie/${id}`} onClick={() => { handleCloseModal(); scrollToTop() }} className='min-w-fit flex items-center space-x-1 py-2 px-5 md:px-9 rounded-xl border border-green-500 text-green-600 cursor-pointer transition-all duration-500 ease-linear hover:bg-green-600 hover:text-white justify-center'>
                            {/* <PiPlayCircleFill/> */}
